...
class MLUPowerDifferenceOp : public MLUOpKernel {
 public:
  ...
  void ComputeOnMLU(OpKernelContext* ctx) override {
        ...
        // broadcast the output dim
        // e.g. tensor_a: n h w c   tensor_b: w c   output: (n h) w c
        if (max_dims - i > dims_a) {
            shape.InsertDim(i, b.dim_size(i));
        } else if (max_dims - i > dims_b) {
            shape.InsertDim(i, a.dim_size(i));
        } else {
            // align the ouput shape
            // e.g. tensor_a: n h w c_a   tensor_b: n h w c_b   c_a > c_b   output: n h w c_a
            ...
            // if the dims of input tensors are not equal and none of which equal to 1, input tensors is invalid
            OP_REQUIRES(ctx, dim_a == dim_b || dim_a == 1 || dim_b == 1,
                errors::InvalidArgument(
                "PowerDifferenceOp: Tensor Shape Not Supported, input1: ",
                a.shape().DebugString(),
                ", input2: ", 
                b.shape().DebugString()));
